...
function EventPage() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-background">
      ...
      <Belt className="mt-14" items={event.highlights} />
      ...
      <section className="relative z-10 mx-auto mt-14 max-w-5xl px-5 sm:px-8">
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-3">
          {event.highlights.map((h) => (
            <div key={h} className="rounded-2xl glass px-5 py-4">
              <span className="block h-1 w-8 rounded-full" style={{ background: accent }} />
              <p className="mt-3 font-display text-sm font-semibold">{h}</p>
      ...
      <section className="relative z-10 mx-auto mt-16 max-w-5xl px-5 sm:px-8">
        ...
        <div className="mt-6 columns-1 gap-4 sm:columns-2 lg:columns-3">
          {event.photos.map((src, i) => (
            <figure
              key={src}
              className="animate-rise mb-4 break-inside-avoid overflow-hidden rounded-2xl border border-border/70"
